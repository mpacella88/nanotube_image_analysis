...
#updating this to account for inter-network joining....
#I think the simplest thing to do would be to first decide whether an inter or intra network joining event will occur and then go from there
#we have the rtot of all possible inter network joinging events, just need to calclate the rtot of all intra-network joining events 
...
def compute_rtot_for_individual_network(network):
	...
	connectivity_dict = nx.get_node_attributes(network, 'arms')
	adapter_dict = nx.get_node_attributes(network, 'adapter')

	#count valid positive and negative tube ends for the network
	valid_positive_attachment_nodes = []
	valid_negative_attachment_nodes = []

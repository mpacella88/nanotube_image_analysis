...
def process_tiff_stacks(filename):
	...
	for image in cy3_image_stack:
		...
		if (i+1)%2 != 0:
			...
			for region in regionprops(label_image):
				...
				if region.area/tube_width >= length_cutoff and region.eccentricity >= eccentricity_cutoff:
					...
					#print contour 
					ordered_coords = generate_ordered_coords_from_skeleton(skeleton, region_endpoints[0], region_endpoints[1])

					#this code is used to calculate the angle simply based on the endpoint to endpoint vector
					...
					'''
					mean_tangent = calc_mean_tangent_from_ordered_coords(ordered_coords)
					mean_tangents.append(mean_tangent)
		...
		f1=open('mean_tangent_differences.dat','a')
		for mean_tangent in mean_tangents:
			print >>f1, mean_tangent
		f1.close()

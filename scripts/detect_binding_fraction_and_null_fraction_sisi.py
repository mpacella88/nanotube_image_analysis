...
def detect_ab_null_membrane_fraction(ab_filename, seed_filename):
	...
	row_indices, column_indices = np.nonzero(edges_open)
	...
	print "culled binding fraction is: ", culled_binding_fraction

	#now we want to generate a random array of seeds (with the same number of seeds as the original arryay)
	#these seeds will be randomly placed withing the "membrande", that is, within some distance of a an edge pixel for the cell
	#this should be possible by considering circles around each edge pixel and all pixels within the circle unless they are already 
	#in the "membrane"
	...
	'''for edge_point in edge_indices:
		...
		c = plt.Circle((edge_point[1], edge_point[0]), membrane_radius, color='red', linewidth=2, fill=False)
		ax.add_patch(c) '''

		

	#ax.imshow(img_membrane, cmap = 'Blues')
	#io.imsave('membrane_processed.pdf', img_as_uint(img_membrane))
	plt.savefig("plots/"+ab_filename+"processed_with_membrane.pdf")

	#now we need a list of all pixel indices in membrane_img that are equal to 1
...
for cell_dir in cell_dirs:	
	...
	for i in range(len(ab_file_list)):
		...
		try:
			attached_seed_percentage, null_fraction = detect_ab_null_membrane_fraction(ab_full_file, seed_full_file)
	...
	f1=open(cell_dir+'_null_fractions.dat','w+')
	for percentage in null_fractions:
		print >>f1, percentage
	f1.close()

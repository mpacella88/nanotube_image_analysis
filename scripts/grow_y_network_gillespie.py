...
def initialize_system():
	...
	n_2arm = 20
	n_3arm = 20
	for i in range(n_1arm):
		system.add_node(system.number_of_nodes()+1, arms = 1)
	for i in range(n_2arm):
		system.add_node(system.number_of_nodes()+1, arms = 2)
	for i in range(n_3arm):
		system.add_node(system.number_of_nodes()+1, arms = 3)
...
def joining_rate(network_1, network_2):
	...
	#that is if either on does not have any nodes with arms>connections
...
def perform_joining(network_1, network_2, system):
	...
	#hack for now, the edge is added between two random nodes
	connectivity_dict1 = nx.get_node_attributes(network_1, 'arms')
	connectivity_dict2 = nx.get_node_attributes(network_2, 'arms')

...
def count_abs_in_membrane(ab_filename, seed_filename):
	...
	sd_pairwise_distance = np.std(pairwise_distances)
	...
	return len(culled_ab_points_array_on_edge), average_pairwise_distance, sd_pairwise_distance 
...
time_seed_pairwise_distance_stds = []

for time_dir in time_dirs:	
	ab_membrane_counts = []
	ab_file_list = glob.glob(time_dir+"/*z00000006*1.tif")
	seed_file_list = glob.glob(time_dir+"/*z00000006*2.tif")
	print ab_file_list
	...
	for i in range(len(ab_file_list)):
		...
		#try:
		ab_membrane_count, seed_pairwise_distance_avg, seed_pairwise_distance_std = count_abs_in_membrane(ab_full_file, seed_full_file)
	...
	time_seed_pairwise_distance_avgs.append(seed_pairwise_distance_avg)
	time_seed_pairwise_distance_stds.append(seed_pairwise_distance_std)

print "printing sum total of ab counts for time points"
f1=open(time_dir+'_total_membrane_count.dat','w+')
for count in time_ab_membrane_counts:
	print >>f1, count 
f1.close()

print "printing average pairwise distance for time points"
f1=open('pairwise_distance.dat','w+')
for i in range(len(time_seed_pairwise_distance_avgs)):
	dist = time_seed_pairwise_distance_avgs[i]
	print >>f1, dist  
f1.close()

print "printing average pairwise distance std for time points"
f1=open('std_pairwise_distance.dat','w+')
for i in range(len(time_seed_pairwise_distance_stds)):
	dist = time_seed_pairwise_distance_stds[i]
	print >>f1, dist  
f1.close()
#times = []
